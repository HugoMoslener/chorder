...
export default function TaskerProfileScreen() {
  ...
  const earned = myTasks.filter((c) => c.status === 'completed').reduce((sum, c) => sum + c.price, 0);
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.role}>Tasker</Text>
      <View style={styles.ratingRow}>
        <Text style={styles.ratingText}>⭐ {currentUser?.rating}</Text>
      </View>

      <View style={styles.statsRow}>
        <View style={styles.stat}>
          <Text style={styles.statValue}>{currentUser?.completedTasks}</Text>
          <Text style={styles.statLabel}>Lifetime Tasks</Text>
        </View>
        <View style={styles.stat}>
...
const styles = StyleSheet.create({
  ...
  ratingText: { fontSize: 18, fontWeight: '600', color: '#f9a825' },
